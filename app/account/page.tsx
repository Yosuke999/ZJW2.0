...
export default async function AccountPage({ searchParams }: { searchParams: Promise<{ country?: string; language?: string; returnTo?: string }> }) {
  ...
  if (!profile) redirect(`/auth?country=${country}&language=${language}&returnTo=${encodeURIComponent(returnTo)}`);
  return <main className="account-shell shell"><AccountDashboard profile={profile as CustomerProfile} intents={(intents ?? []) as never} country={country} language={language} returnTo={returnTo} /></main>;
}
